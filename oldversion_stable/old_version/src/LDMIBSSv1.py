...
class OnlineLDMIBSS:
    ...
    def find_permutation_between_source_and_estimation(self, S, Y):
        ...
        perm = np.argmax(
            np.abs(self.outer_prod_broadcasting(Y, S).sum(axis=0))
            / (np.linalg.norm(S, axis=0) * np.linalg.norm(Y, axis=0)),
            axis=0,
        )
        return perm
    ...
    def fit_batch_antisparse(
        ...
        for k in range(n_epochs):
            ...
            for i_sample in tqdm(range(samples)):
                ...
                yk, ek = self.run_neural_dynamics_antisparse(
                    yk,
                    yke,
                    My,
                    gamy,
    ...
    def fit_batch_nnantisparse(
        ...
        for k in range(n_epochs):
            ...
            for i_sample in tqdm(range(samples)):
                ...
                # NumberofOutputIterations_ = np.int64(5 + np.min([np.ceil(i_sample / 50000.0),neural_dynamic_iterations]))
                NumberofOutputIterations_ = neural_dynamic_iterations
                yk, ek = self.run_neural_dynamics_nnantisparse(
                    yk,
                    yke,
                    My,
                    gamy,
    ...
    def fit_batch_sparse(
        ...
        for k in range(n_epochs):
            ...
            for i_sample in tqdm(range(samples)):
                ...
                # NumberofOutputIterations_ = np.int64(5 + np.min([np.ceil(i_sample / 50000.0),neural_dynamic_iterations]))
                NumberofOutputIterations_ = neural_dynamic_iterations
                yk, ek = self.run_neural_dynamics_sparse(
                    yk,
                    yke,
                    My,
                    gamy,
    ...
    def fit_batch_nnsparse(
        ...
        for k in range(n_epochs):
            ...
            for i_sample in tqdm(range(samples)):
                ...
                # NumberofOutputIterations_ = np.int64(5 + np.min([np.ceil(i_sample / 50000.0),neural_dynamic_iterations]))
                NumberofOutputIterations_ = neural_dynamic_iterations
                yk, ek = self.run_neural_dynamics_nnsparse(
                    yk,
                    yke,
                    My,
                    gamy,
...
class OnlineWhiten:
    def __init__(
        ...
        if W is not None:
            assert W.shape == (
                x_dim,
                s_dim,
            ), "The shape of the initial guess W must be (x_dim, s_dim) = (%d, %d)" % (
                x_dim,
                s_dim,
            )
            W = W
        ...
        if Winv is not None:
            assert Winv.shape == (s_dim, x_dim), (
                "The shape of the initial guess Winv must be (s_dim, x_dim) = (%d, %d)"
                % (s_dim, x_dim)
            )
            Winv = Winv
        else:
        ...
        if Ro is not None:
            assert Ro.shape == (s_dim, s_dim), (
                "The shape of the initial guess Ro must be (s_dim, s_dim) = (%d, %d)"
                % (s_dim, s_dim)
            )
            Ro = Ro
        else:
    ...
    def compute_overall_mapping(self):
        ...
        We = (
            ...
            * np.linalg.pinv(
                bet * W.T @ W
                + ((1 - lambd) / s_dim * p) * np.eye(s_dim, s_dim)
                - (gam / s_dim) * B
            )
            @ W.T
        )
    ...
    def whiten_transform(self, X):
        assert (
            X.shape[0] == self.W.shape[1]
        ), "Matrix dimensions do not match for whitening matrix W and input signal X"
        return self.W @ X
    ...
    def run_neural_dynamics(
        ...
        for i in range(n_iterations):
            ...
            gradJ = (
                bet * ek + (1 - lambd) / p * skb / s_dim - gam / s_dim * np.dot(B, skb)
            )  # B @ skb
    ...
    def fit_batch_whiten(
        self,
        X,
        n_epochs=1,
        neural_dynamic_iterations=1550,
        neural_lr_start=5,
        neural_lr_stop=1e-2,
        shuffle=True,
        required_SIR=35,
        ...
        for k in range(n_epochs):
            ...
            for i_sample in tqdm(range(samples)):
                ...
                sk, skb, ek = self.run_neural_dynamics(
                    sk,
                    xk,
                    Winv,
                    B,
                ...
                if np.mod(i_sample, debug_iteration_point) == 0:
                    We = (
                        np.linalg.pinv(
                            (bet + (1 - lambd) / p / s_dim) * np.eye(s_dim)
                            - gam / s_dim * B
                        )
                        @ Winv
                        * bet
                    ...
                    if plot_in_jupyter:
                        ...
                        pl.plot(np.array(WhiteFOM))
                        pl.xlabel(
                            f"Number of Iterations / {debug_iteration_point}",
                            fontsize=15,
                        )
                        pl.ylabel("SIR (dB)", fontsize=15)
                        pl.title("SIR Behaviour", fontsize=15)
