        ...
        for j in range(neural_dynamic_iterations):
            ...
            if sparse_dims_list[0][0] != -1:
                for ss, sparse_dim in enumerate(sparse_dims_list):
                    ...
                    if nn_dims[0] != -1:
                        y[np.array(loop_intersection(sparse_dim, nn_dims))] = ReLU(
                            y[np.array(loop_intersection(sparse_dim, nn_dims))]
                            - STLAMBD_list[ss]
                        )
                    STLAMBD_list[ss] = max(
                        STLAMBD_list[ss] + (np.linalg.norm(y[sparse_dim], 1) - 1), 0
                    )
            if signed_dims[0] != -1:
    ...
    def fit_batch_antisparse(
        ...
        for k in range(n_epochs):
            ...
            for i_sample in tqdm(range(samples)):
                ...
                if debugging:
                    if (
                        ((i_sample % debug_iteration_point) == 0)
                        | (i_sample == samples - 1)
                        ...
                        if plot_in_jupyter:
                            ...
                            pl.subplot(1, 2, 1)
                            pl.plot(
                                np.arange(1, len(self.SINR_list) + 1),
                                np.array(SIRlist),
                                linewidth=3,
                                label="SIR",
                            )
            ...
            for i_sample in tqdm(range(samples)):
                ...
                if debugging:
                    if (
                        ...
                        Y_ = self.signed_and_permutation_corrected_sources(
                            Szeromean.T, Yzeromean.T
                        )
                        ...
                        if plot_in_jupyter:
                            ...
                            pl.subplot(1, 2, 1)
                            pl.plot(
                                np.arange(2, len(self.SINR_list) + 2),
                                np.array(SIRlist),
                                linewidth=3,
                                label="SIR",
                            )
...
class PMF:
    ...
    def __init__(
        self, s_dim, x_dim, H=None, set_ground_truth=False, Sgt=None, Agt=None
    ):
        if H is not None:
            assert H.shape == (
                x_dim,
                s_dim,
            ), "The shape of the initial guess H must be (s_dim, x_dim) = (%d,%d)" % (
                s_dim,
                x_dim,
            )
            H = H
...
class OnlineBSM:
    ...
    def __init__(
        ...
        if W is not None:
            if whiten_input_:
                assert W.shape == (s_dim, s_dim), (
                    "The shape of the initial guess W must be (s_dim,s_dim)=(%d,%d) (because of whitening)"
                    % (s_dim, x_dim)
                )
                W = W
            else:
            ...
            if whiten_input_:
                ...
                W = 0.0033 * (
                    W / np.sqrt(np.sum(np.abs(W) ** 2, axis=1)).reshape(s_dim, 1)
                )
            else:
                W = np.random.randn(s_dim, x_dim)
                W = 0.0033 * (
        ...
        if M is not None:
            assert M.shape == (
                s_dim,
                s_dim,
            ), "The shape of the initial guess W must be (s_dim,s_dim)=(%d,%d)" % (
                s_dim,
                s_dim,
            )
            M = M
    ...
    def fit_next_antisparse(
        ...
        D = (1 - beta) * D + mu * np.diag(
            np.sum(np.abs(W) ** 2, axis=1) - np.diag(M @ D @ M)
        )
