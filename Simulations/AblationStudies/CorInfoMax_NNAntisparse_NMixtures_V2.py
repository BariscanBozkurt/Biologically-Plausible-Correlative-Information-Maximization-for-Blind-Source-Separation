...
pickle_name_for_results = "simulation_results_nnantisparse_nmixturesV2.pkl"
...
seed_list = np.array([113 * i for i in range(NumAverages)]) ## Seeds for reproducibility
...
for iter1 in range(NumAverages): ## Loop over number of averages
    ...
    for n_mixtures in NMixture_list:
        ...
        try: # Try Except for SVD did not converge error (or for any other error)
            ...
            modelCorInfoMax = OnlineCorInfoMax( s_dim = s_dim, x_dim = n_mixtures, muW = 30*1e-3, lambday = lambday,
                                                lambdae = lambdae, By = By, Be = Be, neural_OUTPUT_COMP_TOL = 1e-6,
                                                set_ground_truth = True, S = S, A=A[:n_mixtures,:])

            with Timer() as t:
                modelCorInfoMax.fit_batch_nnantisparse( X = Xnoisy[:n_mixtures,:], 
                                                        n_epochs = 1, 
