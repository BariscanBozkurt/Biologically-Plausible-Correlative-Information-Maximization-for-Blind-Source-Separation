...
pickle_name_for_results = "simulation_results_muWSelection_AntisparseV4.pkl"
...
seed_list = np.array([1475452 * i for i in range(NumAverages)]) ## Seeds for reproducibility
########################################################################################
########################################################################################
###                                                                                  ###
###                        SIMULATION                                                ###
...
RESULTS_DF = pd.DataFrame( columns = ['trial', 'seed', 'Model', 'muW', 'SINR', 'SINRlist', 'SNR', 'S', 'A', 'X', 'Wf', 'SNRinp', 'execution_time'])
...
for iter1 in range(NumAverages): ## Loop over number of averages
    ...
    for muWselection in muW_list:
        ...
        try: # Try Except for SVD did not converge error (or for any other error)
            ...
            modelCorInfoMax = OnlineCorInfoMax( s_dim = s_dim, x_dim = x_dim, muW = muWselection, lambday = lambday,
            ...
            CorInfoMax_Dict = {'trial' : trial, 'seed' : seed_, 'Model' : 'CorInfoMax', 'muW': muWselection,
