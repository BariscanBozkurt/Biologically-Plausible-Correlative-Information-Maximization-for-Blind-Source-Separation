...
class RVolMin(BSSBaseClass):
    ###### !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! #####################
    ###### STILL UNDER DEVELOPMENT, NOT WORKING PROPERLY YET #####################
    """
    ...
    """
    def __init__(self, s_dim, x_dim,
                 set_ground_truth = False, S = None, A = None) -> None:
        super().__init__()
    ...
    def fit_batch(self, X, n_iterations = 10000, p = 0.5, Lt = 50, lambda_ = 1, tau = 1e-8, 
                  epsilon = 1e-12, debug_iteration_point = 100, plot_in_jupyter = False):
        ...
        for k in tqdm(range(n_iterations)):
            ...
            C = C + ((q_prev - 1)/q)*(diff)
            C = C - (np.dot(B.T,(np.dot(B, C) - X))/Lt)#(Lt*np.linalg.norm(np.transpose(B)@B, 2)))
            ...
            W = ((p / 2) * (np.sum((X - B @ C) ** 2, axis = 0) + epsilon)**((p - 2)/2))[:,np.newaxis]
            if debugging:
                if ((k % debug_iteration_point) == 0)  | (k == n_iterations - 1):
                    try:
                        ...
                        SINR, SNR, SGG, Y_, P = self.evaluate_for_debug(Wseparator, S, A, C)
                        ...
                        if plot_in_jupyter:
                            ...
                            self.plot_for_debug(SIR_list, SNR_list, P, debug_iteration_point, CforPlot)
